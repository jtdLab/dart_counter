...
class CreateGameEvent with _$CreateGameEvent {
  const factory CreateGameEvent.watchStarted() = WatchStarted;
  const factory CreateGameEvent.receivedGame({
    required Game game,
  }) = ReceivedGame;
  ...
  const factory CreateGameEvent.playerRemoved({required int index}) = PlayerRemoved;
  const factory CreateGameEvent.playerNameUpdated(
      {required int index, required String newName}) = PlayerNameUpdated;
  const factory CreateGameEvent.startingPointsUpdated({required int newStartingPoints}) =
      StartingPointsUpdated;
  const factory CreateGameEvent.modeUpdated({required Mode newMode}) = ModeUpdated;
  const factory CreateGameEvent.sizeUpdated({required int newSize}) = SizeUpdated;
  const factory CreateGameEvent.typeUpdated({required Type newType}) = TypeUpdated;
  const factory CreateGameEvent.gameStarted() = GameStarted;
  const factory CreateGameEvent.throwPerformed({required Throw t}) = ThrowPerformed;
  ...
  const factory CreateGameEvent.dartBotTargetAverageUpdated(
      {required int newTargetAverage}) = DartBotTargetAverageUpdated;
}
